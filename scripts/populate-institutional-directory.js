...
const dbPath = path.resolve(__dirname, '../../public_html/storage/database/campus.sqlite');
const db = new Database(dbPath);

console.log('Inizializzazione tabella institutional_directory in:', dbPath);
...
// 1. Inserisci i 58 contatti del CENSUS ufficiale
for (const c of CENSUS_CONTACTS) {
  ...
  insertStmt.run(
    ...
    c.id.split('-')[0],
    c.category || 'GENERALE',
    c.target_role || 'Presidente e Segreteria',
    c.email || '',
    c.pec || '',
    c.phone || '',
    c.website || '',
...
// 2. Inserisci Grandi Ordini e Collegi Territoriali Strategici
...
for (const o of territorialOrders) {
  insertStmt.run(
    ...
    o.id.split('-')[1],
    o.sector,
    'Presidente e Segreteria Generale',
    o.email,
    o.pec,
    '',
    o.site,
    ...
    o.prov,
    '',
    'Iscritti all\'albo provinciale convocabili per docenze e CFP',
    'ATTIVO'
  );
  count++;
}

// 3. Inserisci Grandi Imprese Nazionali & Regionali (Corporate Academy & Formazione Finanziata)
...
for (const c of corporateEntities) {
  insertStmt.run(
    ...
    c.id.replace('CORP-', ''),
    c.sector,
    'Direzione Risorse Umane & Corporate Academy',
    c.email,
    c.pec,
    '',
    c.site,
    c.reg === 'NAZIONALE' ? 'NAZIONALE' : 'REGIONALE',
    c.reg,
    c.prov,
    '',
    c.desc,
...
// 4. Inserisci Pubbliche Amministrazioni, Ministeri ed Enti Territoriali (Convenzioni e PNRR)
...
for (const p of publicEntities) {
  insertStmt.run(
    ...
    p.id.replace('PA-', ''),
    p.sector,
    'Direzione Generale e URP',
    p.email,
    p.pec,
    '',
    p.site,
    p.reg === 'NAZIONALE' ? 'NAZIONALE' : 'REGIONALE',
    p.reg,
    p.prov,
    '',
    p.desc,
...
console.log(`✅ Inseriti ${count} record qualificati nella tabella institutional_directory!`);

const stats = db.prepare(`
SELECT category_group, count(*) as c 
FROM institutional_directory 
GROUP BY category_group
`).all();

console.log('Riepilogo per gruppo:', stats);
db.close();
